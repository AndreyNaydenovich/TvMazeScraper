...
namespace TvMazeScraper.Integration.Jobs
{
    internal class ShowSynchronizationHostedService : IHostedService, IDisposable
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(async (state) => { await DoWork((CancellationToken)state); }, cancellationToken, TimeSpan.Zero, TimeSpan.FromSeconds(120));
        ...
        public async Task DoWork(CancellationToken cancellationToken)
        {
            ...
            try
            {
                await _showSynchronization.StartSynchronizationAsync(cancellationToken);
            }
            catch (Exception ex)
            {
